...
var favPlaces = [AnyObject]()

//update data function
func getLatestPlaces(){
    if(NSUserDefaults.standardUserDefaults().objectForKey("places") != nil){
        favPlaces = NSUserDefaults.standardUserDefaults().objectForKey("places")! as NSArray
    }else{
        favPlaces = []
    }
}
...
class TableViewController: UITableViewController {
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if(favPlaces.count > 0){
            cell.textLabel?.text = favPlaces[indexPath.row] as NSString
        }
        return cell
    }
